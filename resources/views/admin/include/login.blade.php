...
<body class="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-700 via-slate-800 to-slate-900 p-4">
  <div class="w-full max-w-md">
    ...
    <div class="login-card rounded-3xl shadow-2xl p-8 sm:p-10 bg-white/95 backdrop-blur-sm border border-white/20">
      ...
      <!-- form -->
      <form class="mt-7 space-y-6" method="POST" action="{{ route('system.login') }}">
        @csrf
        ...
        <div class="flex items-center justify-between">
          <label class="flex items-center gap-2 text-sm text-slate-600 cursor-pointer">
            <input type="checkbox" name="remember" id="remember"
              class="custom-check h-4 w-4 rounded border-slate-300 text-blue-800 focus:ring-2 focus:ring-blue-700 focus:ring-offset-0 transition" />
            <span>Remember me</span>
          </label>
          <a href="#" class="text-sm font-medium text-slate-700 hover:text-slate-900 transition link-underline">Reset password</a>
        </div>
      ...
      <!-- footer links -->
      <div class="mt-8 text-center border-t border-slate-200/70 pt-6">
        <p class="text-sm text-slate-500">
          Don't have an account?
          <a href="#" class="font-semibold text-slate-800 hover:text-slate-900 transition link-underline">Create Account</a>
        </p>
        <p class="mt-4 text-xs text-slate-400/80 tracking-wide">
          <span class="inline-block bg-slate-100 px-2 py-0.5 rounded-full text-slate-500">🔒 secure access</span>
        </p>
      </div>

    </div>
    <!-- subtle footer -->
    <p class="text-center text-[0.65rem] text-white/30 mt-6 tracking-wider">PORTAL · ADMIN</p>
  </div>
</body>
</html>
